...
func init() {
	...
	engine.OnFullMatch("呆呆瓜").SetBlock(true).Handle(func(ctx *zero.Ctx) {
	...
	engine.OnFullMatchGroup([]string{"猪", "猪猪"}).SetBlock(true).Handle(func(ctx *zero.Ctx) {
	...
	engine.OnFullMatch("唯爱爱").SetBlock(true).Handle(func(ctx *zero.Ctx) {
		ctx.Send(message.At(tadanoai))
	})
}
