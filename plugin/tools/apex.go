package tools // Package tools only accessible to PC
...
func init() {
	accessload := fcext.DoOnceOnSuccess(func(ctx *zero.Ctx) bool {
		apexToken := engine.DataFolder() + "apexToken.txt"
	...
	engine.OnFullMatch("/apex", accessload, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		apexToken := engine.DataFolder() + "apexToken.txt"
...
func InsertPlayUser(userID string, playID string) (err error) { // 一次轮询，填入对应玩家信息
	...
	playList := engine.DataFolder() + "apex_playerlist.json"
...
func LoadUserPlayID(ctx *zero.Ctx, userID string) (playUID string) {
	...
	playListJson := engine.DataFolder() + "apex_playerlist.json"
...
func QueryUserNameToID(ctx *zero.Ctx, name string) (UserName string, UserID string) {
	apexTokenByte, _ := os.ReadFile(engine.DataFolder() + "apexToken.txt")
