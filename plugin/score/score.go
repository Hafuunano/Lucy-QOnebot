...
func init() {
	...
	engine.OnFullMatchGroup([]string{"签到", "打卡"}, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			...
			picFile := cachePath + strconv.FormatInt(uid, 10) + today + ".png" // send directly.
			_ = initPic(picFile)
			...
			if now.Hour() > 6 && now.Hour() < 19 {
				...
				dayGround.DrawStringWrapped(strconv.Itoa(si.Count), 1000, 255, 1, 1, 0, 1.3, gg.AlignCenter)       // draw second part
				dayGround.DrawStringWrapped(strconv.Itoa(coinsGet), 220, 370, 1, 1, 0, 1.3, gg.AlignCenter)        // draw third part
				dayGround.DrawStringWrapped(strconv.Itoa(si.Coins), 720, 370, 1, 1, 0, 1.3, gg.AlignCenter)        // draw forth part
				...
				dayGround.DrawRectangle(70, 570, 600*(1-currentResult), 50) // draw rectangle part2
				dayGround.Fill()
				...
				nightGround := gg.NewContext(1920, 1080)
				...
				nightGround.DrawStringWrapped(strconv.Itoa(si.Count), 990, 275, 1, 1, 0, 1.3, gg.AlignCenter)        // draw second part
				nightGround.DrawStringWrapped(strconv.Itoa(coinsGet), 225, 360, 1, 1, 0, 1.3, gg.AlignCenter)        // draw third part
				nightGround.DrawStringWrapped(strconv.Itoa(si.Coins), 720, 360, 1, 1, 0, 1.3, gg.AlignCenter)        // draw forth part
				// level array with rectangle work.
				rankNum := getLevel(score)
				RankGoal := rankNum + 1
				achieveNextGoal := levelArray[RankGoal]
				...
				nightGround.Fill()
				nightGround.SetRGB255(255, 255, 255)
				nightGround.DrawRectangle(70, 570, 600*(1-currentResult), 50) // draw rectangle part2
				nightGround.Fill()
				nightGround.SetRGB255(255, 255, 255)
