...
func init() {
	engine.OnRegex(`^\/pgr\sbind\s(.*)$`).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		...
		data, err := web.GetData("http://localhost:9090/saveUrl/" + getSessionID)
		if err != nil {
			ctx.SendChain(message.Reply(ctx.Event.MessageID), message.Text("出现错误： 请稍后再尝试"))
			return
		}
	...
	engine.OnFullMatch("/pgr b19").SetBlock(true).Handle(func(ctx *zero.Ctx) {
		...
		getUrlData, err := web.GetData("http://localhost:9090/saveUrl/" + getDataSession)
		if err != nil {
			ctx.SendChain(message.Reply(ctx.Event.MessageID), message.Text("出现错误URLDATA：", err))
			return
		}
		var PhigrosSaves saveURLData
