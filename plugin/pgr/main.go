...
func init() {
	...
	engine.OnFullMatch("/pgr b19").SetBlock(true).Handle(func(ctx *zero.Ctx) {
		...
		phidata := GetRequestInfo(ctx.Event.UserID)
		...
		getMainBgRender.DrawString("Player: "+phidata.Content.PlayerID, 1490, 300)
		getMainBgRender.DrawString("RankingScore: "+strconv.FormatFloat(phidata.Content.RankingScore, 'f', 3, 64), 1490, 380)
		getMainBgRender.DrawString("ChanllengeMode: ", 1490, 460) // +56
		getColor, getLink := GetUserChallengeMode(phidata.Content.ChallengeModeRank)
		if getColor != "" {
		...
		_ = getMainBgRender.SavePNG("result.png")
