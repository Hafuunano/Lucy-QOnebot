...
func init() {
	...
	engine.OnRegex(`^[! ！]pgr\sroll\s(\d+)`).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		...
		getMainBgRender.SetColor(color.White)
		getMainBgRender.Fill()
		getMainBgRender.DrawString("Generated By Lucy (HiMoYoBOT) | Designed By Eastown | Data From Phigros Unlimited API & Phigros Library Project", 10, float64(5100+getOverFlowNumber*200+100))
		tureResult := getMainBgRender.Image()
		var buf bytes.Buffer
		err = jpeg.Encode(&buf, tureResult, nil)
		if err != nil {
