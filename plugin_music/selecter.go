...
var limit = rate.NewManager(time.Minute*3, 6)
...
func cloud163(keyword string) message.MessageSegment {
	headers := http.Header{
		"User-Agent":   []string{"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36 Edg/92.0.902.78"},
	}
	data := url.Values{
		"keywords":      []string{keyword},
	}
	// 搜索音乐信息 第一首歌
	info := gjson.ParseBytes(netPost("https://nemapi.windis.xyz/search", data, headers)).Get("result.songs.0")
	// 返回音乐卡片
	return message.CustomMusic(
		fmt.Sprintf("https://y.music.163.com/m/song?id=%d", info.Get("id").Int()),
		fmt.Sprintf("https://music.163.com/song/media/outer/url?id=%d.mp3", info.Get("id").Int()),
		info.Get("name").Str,
